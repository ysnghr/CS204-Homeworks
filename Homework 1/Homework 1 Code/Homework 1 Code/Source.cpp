...
// this function makes vector of two integers from the file stream
...
// here I am checking the two integers that were given in the file
// one of the input check for this project
...
// here I printed vector of vector of strings on console
...
// checks whether the cell occupied by another word
...
void placeWordonVec(vector<string> givenData, vector<vector<string>>& vec, vector<int> rowAndcol)
{
	...
	// looping over the word from first index to end
	for (int i = 1; i < givenData[0].length(); i++)
	{
		...
		else
		{
			...
			if (givenData[4] == "CCW")
			{
				...
				if (placed == false)
				{
					cout << "\"" << givenData[0] <<"\" could not be put into the matrix with given starting point: "<< point[0] << "," << point[1]
					<<" direction: "<< givenData[3] <<" orientation: " << givenData[4] << endl;
					return;
				}
			}
